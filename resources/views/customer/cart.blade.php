                            ...
                            <tbody>
                                @if ($user)
                                    @foreach ($cart as $cart)
                                        ...
                                        <form action="{{ route('cartdata') }}" method="POST"
                                            enctype="multipart/form-data">
                                            @csrf
                                            <input type="hidden" name="iduser" value="{{ $cart->user->iduser}}">
                                            <input type="hidden" name="cart[{{ $loop->index }}][cid]"
                                                value="{{ $cart->cid }}">
                                            <input type="hidden" name="cart[{{ $loop->index }}][item_price]"
                                                value="{{ $cart->item->item_price }}">
                                            <?php $total = 0; ?>
                                            <tr class="text-center">
                                                <td class="product-remove"><a href="{{route('cartdelete',$cart->cid)}}"><span
                                                            class="ion-ios-close"></span></a></td>
                                                <td><img src="{{ asset('/storage/' . $cart->item->item_image) }}"
                                                        height="50px"></td>
                                                <td class="product-name">
                                                    <h3>{{ $cart->item->item_name }} </h3>
                                                </td>
                                                <td class="price">₹ {{ $cart->Item->item_price }} </td>
                                                <td class="quantity">
                                                ...
                                                <td class="total">₹ {{ $cart->item->item_price }} </td>
                                            </tr>
                                    @endforeach
